...
vec3 Get_HDR_Color(Ray r)
{
	...
	vec4 texColor = texData;//RGBEToLinear(texData);

	return texColor.rgb;// * uHDRI_Exposure;
}
...
//-----------------------------------------------------------------------------------------------------------------------------
{
	...
	for (int bounces = 0; bounces < 6; bounces++)
	{
		...
		if (intersection.t == INFINITY)
		{
			...
			if ( !bounceIsSpecular )
			{
				weight = dot(r.direction, uSunDirection) < 0.98 ? 1.0 : 0.0;
			...
			if (bounceIsSpecular)
			{
				...
				// if (dot(r.direction, uSunDirection) > 0.8)
				// {
